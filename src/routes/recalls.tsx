...
import { classLabel } from "@/lib/server/fda";
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        body="Live food recalls. If a scan matches a product code on this list, the pack shows a stop. We do not invent lots for clean shelves."
      />
      <img
      ...
      />
      {rows.length === 0 ? (
        <p className="mt-6 text-[15px] text-muted">The feed is quiet or unreachable. A scan still checks the date on the pack code.</p>
      ) : (
        <ul className="mt-6 space-y-3">
          {rows.map((r) => (
            <li key={r.eventId + r.product.slice(0, 24)} className="rounded-xl bg-surface p-4 shadow-[var(--shadow-border)]">
              ...
              {r.where ? <p className="mt-2 text-xs text-muted">Where: {r.where.slice(0, 160)}</p> : null}
              {r.quantity ? <p className="mt-1 text-xs text-muted">How much: {r.quantity}</p> : null}
            </li>
          ))}
        </ul>
      )}
      <p className="mt-8 text-sm text-muted">
        Source: U.S. FDA. Always check the pack and the{" "}
